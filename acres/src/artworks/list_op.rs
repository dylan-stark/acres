...
use crate::{AcresError, api::Api, artworks::List, config::Config};
...
impl ListOp {
    /// Sets API.
    ///
    /// Use this when you want to directly construct the list operation, but also want customize
    /// the API.
    ///
    /// # Examples
    ///
    /// ```
    /// use acres::Api;
    /// use acres::artworks::ListOp;
    ///
    /// let api = Api::builder().use_cache(false).build();
    /// ListOp::default().api(api).get();
    /// ```
